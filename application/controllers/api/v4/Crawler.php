...
class Crawler extends STEVEN_Controller
{
    ...
    protected $_client;
    protected $_data_gold_price;
    protected $_data_exchange_rate;
    protected $_data_keno;
    ...
    public function __construct()
    {
        ...
        $this->load->model(array('result_model', 'category_model', 'gold_price_model', 'exchange_rate_model', 'keno_model'));
        $this->_data_category = new Category_model();
        $this->_data_result = new Result_model();
        $this->_data_gold_price = new Gold_price_model();
        $this->_data_exchange_rate = new Exchange_rate_model();
    ...
    public function testDB() {
        $this->sendBot("test send bot");
    }

    public function crawlerAllResult()
    {
        /*===>>> START CRAWLER RESULT MINHNGOC.NET <<<===*/
        //$this->crawler_result_mb_minhngocnet('31-12-2004'); //01-01-2005
        //$this->crawler_result_mtmn_minhngocnet('xsmt',);
        //$this->crawler_result_mtmn_minhngocnet('xsmn',);
        /*===>>> END CRAWLER RESULT MINHNGOC.NET <<<===*/

        /*===>>> START CRAWLER RESULT XOSO.ME <<<===*/
        //$this->crawler_result_mb_xosome('9-2-2002'); //01-01-2005
        //$this->crawler_result_mt_xosome('xsmt','31-12-2008');
        //$this->crawler_result_mn_xosome('xsmn','31-12-2008');
        /*===>>> END CRAWLER RESULT XOSO.ME <<<===*/
        //$this->crawlListResultMienBac('17-09-2019'); //30-09-2005
        //$this->crawlListResultMTMN('xsmt','28-10-2007'); //13-08-2006
        //$this->crawlListResultMTMN('xsmn','30-09-2005'); //30-09-2005
        ///
        //$this->crawlListResultMTMNFixListDay('xsmt', '13-08-2006', '03-02-2008');
        /* cái trên là fix listngược tỉnh ngày chủ nhật , của miền trung (crawl : kontum - khanh hoa => moi khanh hoa-kon tum) */
        //$this->crawlResultPowerMega('Power');
        //$this->crawlResultPowerMega('Mega');
        //$this->crawlResultMax3D();
        //$this->crawlResultMax4D();
        //$this->crawlLivePowerMega('Mega');

        echo "------Crawler live POWER--------\n";
        $this->crawlLivePowerMega_Vietlottvn('Power');
        echo "------Completed crawler live POWER--------\n";

        echo "------Crawler live MEGA--------\n";
        $this->crawlLivePowerMega_Vietlottvn('Mega');
        echo "------Completed crawler live MEGA--------\n";

        echo "------Crawler live MAX3D--------\n";
        $this->crawlLiveResultMax3d();
        echo "------Completed crawler live MAX3D--------\n";

        echo "------Crawler live MAX4D--------\n";
        $this->crawlLiveResultMax4d();
        echo "------Completed crawler live MAX4D--------\n";
...
//            $this->crawlLiveResultMTMNMinhNgoc("https://www.minhngoc.net/xo-so-truc-tiep/mien-trung.html");
...
//                $this->crawlerLive4d_atrungroi();
    ...
    /*/////////////*/
    private function crawlLiveResultMTMN($url)
    {
        $date = date('Y-m-d');
        try {
                    ...
                    if (strtotime($getDate) == strtotime(date('Y-m-d'))) {
                        ...
                        if ($crawler->filterXPath('//*[@data-id="kq"]')->count() > 0) {
                            ...
                            if (!empty($listCategory)) foreach ($listCategory as $key => $category_title) {
                                $category_id = $this->_data_result->getCategoryBySlug($category_title);
                            ...
                            if (!empty($data_res)) foreach ($data_res as $category_id => $oneResult) {
                                ...
                                if (!empty($getDataToday)) {
                                    ...
                                    //covid
//                                    if (!in_array($category_id, [13, 9, 16, 18])) {
//                                        $lastId = $this->saveResult([$data]);
//                                    }
                                    $lastId = $this->saveResult([$data]);
                                    if (empty($lastId)) die("Insert data into db errror ! \n");
    ...
    private function crawlLiveResultMTMNMinhNgoc($url)
    {
        $date = date('Y-m-d');
        try {
            $crawler = $this->_client->request('GET', $url);
            ...
            if (empty($response->getContent())) $running = false;
                ...
                if ($getDate == date('d/m/y')) {
                    if ($crawler->filterXPath('//*[@id="box_tructiepkqxs"]//table[position()=1]')->count() > 0) {
                        $listCategory = $crawler->filterXPath('//*[@id="box_tructiepkqxs"]//table[position()=1]//td[@class="tinh"]')->each(function ($category) {
                            return trim($category->text());
                        });
                        ...
                        if (!empty($listCategory)) foreach ($listCategory as $key => $category_title) {
                            $category_id = $this->_data_result->getCategoryBySlug($category_title);
                            if ($category_id == false) $running = false;
                            $dataResult = $crawler->filterXPath('//*[@id="box_tructiepkqxs"]//div[@class="content"]//table[position()=1]/tbody/tr/td[position()=2]//table[position() = ' . ($key + 2) . ']//tbody//tr[position() > 2]/td')->each(function ($result) {
                                $arrNumber = $result->filterXpath('//div')->each(function ($number) {
                                    if ($number->filterXpath('//span')->count() > 0) return "";
                                    else return trim($number->text());
                                });
                                return $arrNumber;
                            });
                            $data_res[$category_id] = $dataResult;
        ...
        try {
            ...
            if (empty($response->getContent())) $running = false;
                ...
                if ($crawler->filter("#$text")->count() > 0) {
                    if ($crawler->filter("#$text i.imgloadig")->count() == 0) {
                        $date = $crawler->filter('h2')->text();
                        preg_match('/\d{1,2}-\d{1,2}-\d{4}/',$date,$date);
                        $date = $date[0];
                        if (strtotime(str_replace('/', '-', $date)) === strtotime(str_replace('/', '-', $datenow))) {
                            ...
                            $data_info = [$period,
                                [[[$data_res[3]],[$data_res[10]],[$data_res[16]],[$data_res[26]]],
                                    $data_info[0]]];
                            $data_res = [[$data_res[1],$data_res[2]],
            ...
            if (empty($response->getContent())) $running = false;
                ...
                if ($getDate == date('d/m/Y', strtotime('+0 days'))) {
                    if ($crawler->filterXPath('//*[@class="content"]')->count() > 0) {
                        ...
                        if ($crawler->filterXPath('//*[@class="bkqmienbac"]//table//table//tbody//tr[position()=1]//td[position()=2]//div//span[@id="loaive_46"]')->count() > 0) {
                            $ma_db = $crawler->filterXPath('//*[@class="bkqmienbac"]//table//table//tbody//tr[position()=1]//td[position()=2]//div//span[@id="loaive_46"]')->html();
                            $ma_db = explode('-', $ma_db);
                        } else $ma_db = [""];
        ...
        if (!empty(preg_match_all("/<tr>(.*?)<\/tr>/si", $matches[0][0], $matches))) {
            foreach ($matches[1] as $k => $item) {
                if (!empty(preg_match_all("/<td (.*?)>(.*?)<\/td>/si", $item, $matches))) {
                    ...
                    $buy_price = trim(strip_tags($buy_price[0])); $buy_price = str_replace(',', '', $buy_price);

                    preg_match("/<span(.*?)>(.*?)<\/span>/si", $matches[0][2], $transfer_price);
                    $transfer_price = trim(strip_tags($transfer_price[0])); $transfer_price = str_replace(',', '', $transfer_price);

                    preg_match("/<span(.*?)>(.*?)<\/span>/si", $matches[0][3], $sell_price);
                    $sell_price = trim(strip_tags($sell_price[0])); $sell_price = str_replace(',', '', $sell_price);
...
//        header("Refresh:0; url=".$url_next);
    ...
    public function crawlerKeno_minhchinh($page = '')
    {
        ...
        $data = curl_exec($ch);
        curl_close($ch);
        $html = \simplehtmldom_1_5\str_get_html($data);
        $data_update = [];
        foreach ($html->find('.wrapperKQKeno') as $item) {
            $period = $item->find('.kyKQKeno', 0)->plaintext;
            $period = str_replace('#', '', $period);
            $displayed_time = $item->find('.timeKQ', 0)->plaintext;
            $displayed_time = str_replace('/', '-', $displayed_time);
            $displayed_time = date('Y-m-d H:i:s', strtotime($displayed_time));
            $result = [];
