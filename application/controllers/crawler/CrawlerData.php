...
class CrawlerData extends STEVEN_Controller
{
    protected $_client;
    ...
    // https://vietlott.vn/vi/trung-thuong/ket-qua-trung-thuong/view-detail-keno-result?id=0084066
            ...
            try {
                ...
                if (empty($response->getContent())) {
                    ...
                    if ($crawler->filterXPath('//*[@class="day_so_ket_qua_v2"]')->count() > 0) {
                        ...
                        $arrResult_active = $crawler->filterXPath('//*[@class="tab-content"]//div[@class="tab-pane active"]')->each(function ($para) {
                        ...
                        $arrResult = array_merge($arrResult_active, $arrResult);
                        $data = [
                            'period' => $id,
                            'displayed_time' => date('Y-m-d', strtotime(str_replace('/', '-', $date))),
                            'result' => json_encode($arrNumber),
                            'info_keno' => json_encode($arrinfo_keno),
                            'info_reward' => json_encode($arrResult)
                        ];

                        dd($data);

                        if ($this->_data_result->checkExistByField('period', $id, 'st_keno')) {
                            echo "Record period $id exists \n";
                        } else {
                            // $lastId = $this->_data_result->save($data, 'st_keno');
                            // if (empty($lastId)) die("Insert data into db errror ! \n");

                            echo "\n => url: $url \n ";
                            echo "Inserted result displayed_time => $date , period => $id\n ";
                        };
                    } else {
                        $running = false;
                        echo "\n URL not empty \n";
    ...
    public function keno2()
    { // 84066
        $running = true;
        for ($id = 87021; $id > 5415; $id--) {
            try {
                $period = substr('0000000' . $id, -7, 7);
                $url = "https://xosominhngoc.net.vn/kqxs-keno/ky-quay-{$period}";
                $crawler = $this->_client->request('GET', $url);
                $response = $this->_client->getResponse();
                if (empty($response->getContent())) {
                    $running = false;
                    ...
                    if ($crawler->filterXPath('//*[@class="xskeno"][position() = 1]')->count() > 0) {
                        echo "\n URL empty \n";
                        if ($crawler->filterXPath('//*[@class="xskeno"][position() = 1]//span[@class="ngay"]//a')->count() > 0) $date = $crawler->filterXPath('//*[@class="xskeno"][position() = 1]//span[@class="ngay"]//a')->text();
                        ...
                        $data = [
                            'period' => $id,
                            'displayed_time' => date('Y-m-d H:i:s', strtotime(str_replace('/', '-', $date))),
                            'result' => json_encode($arrNumber),
                        ...
                        if ($this->_data_result->checkExistByField('period', $id, 'st_keno')) {
                            echo "Record period $id exists \n";
                            $lastId = $this->_data_result->update(['period' => $id], $data, 'st_keno');
    ...
    public function debug()
    {
        for ($i = 1; $i < 100; $i++) {
            ...
            $date = "2021-09-20";
            $date = strtotime($date);
            $date = strtotime("-$i2 day", $date);
            $date = date('j-n-Y', $date);
            $url = "https://xoso.me/xsmn-$date-ket-qua-xo-so-mien-nam-ngay-$date.html";
    ...
    private function crawlLiveResultMTMN($url, $date = null)
    {
        ...
        try {
            ...
            if (empty($response->getContent())) $running = false;
                ...
                //if ($crawler->filterXPath('//*[@id="kq"]//table//tbody//tr[position()>1]//td//div[@class="cl-rl"]')->count() > 0 || $crawler->filterXPath('//*[@id="kq"]//table//tbody//tr[position()>1]//td//div[@class="imgloadig"]')->count() > 0 || date('i', time()) < 50) {
                $textDate = $crawler->filterXPath('//h2')->text();
                preg_match('/\d{1,2}-\d{1,2}-\d\d\d\d/', $textDate, $viewDate);
                $getDate = end($viewDate);
                if (strtotime($getDate) == strtotime($date)) {
                    if ($crawler->filterXPath('//*[@data-id="kq"]')->count() > 0) {
                        $listCategory = $crawler->filterXPath('//*[@data-id="kq"]//table//tbody//tr[position()=1]//th//a')->each(function ($category) {
                            return trim($category->text());
                        ...
                        $data_res = [];
                        if (!empty($listCategory)) foreach ($listCategory as $key => $category_title) {
                            $category_id = $this->_data_result->getCategoryByCode($listCategoryCode[$key], $category_title, $this->toSlug(str_replace("TP ", "", $category_title)));
                            dump($category_id);
                            if ($category_id == false) $running = false;
                            $dataResult = $crawler->filterXPath('//*[@data-id="kq"]//table//tbody//tr[position()>1]/td[position()=' . ($key + 2) . ']')->each(function ($result) {
                                $arrNumber = $result->filterXPath('//div')->each(function ($number) {
        ...
        try {
            ...
            if (empty($response->getContent())) {
                ...
                if ($crawler->filterXPath('//*[@class="xskeno"][position() = 1]')->count() > 0) {
                    ...
                    $date = $crawler->filterXPath('//*[@class="xskeno"][position() = 2]//span[@class="ngay"]')->text();
                    $date = explode(' ', $date);
                    $date = date('Y-d-m H:i:s', strtotime("$date[0] $date[1]"));
