...
class Category extends Admin_Controller
{
    ...
    {
        $data['heading_title'] = "Quản lý page Soi cầu";
        $data['heading_description'] = "Danh sách page Soi cầu";
        $this->get_list($data);
    }
    ...
    public function ajax_list()
    {
        ...
        if (!empty($listData)) foreach ($listData as $category) {
            if (empty($queryFilter) && $type !== 'weekday') {
                ...
                $item = $category;
                $title = $category->meta_title . '<i class="fa fa-fw fa-caret-right"></i>' . $category->title;
            }
            
            switch ($type):
                case 'lottery':
                    $link = getUrlCategoryCode($item);
                    break;
                default:
